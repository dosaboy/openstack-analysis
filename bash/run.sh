...
if [[ -n $HOST_OVERRIDE ]]; then
    ...
    find results_data -name \*.csv| grep $HOST_OVERRIDE| xargs -l python3 $SCRIPT_ROOT/../python/plot.py
else
    rm -rf graphs/*
    find results_data -name \*.csv| xargs -l python3 $SCRIPT_ROOT/../python/plot.py
fi
